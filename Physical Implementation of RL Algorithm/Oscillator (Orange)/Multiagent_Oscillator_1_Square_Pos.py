...
Omega = np.pi/4 # Frequenct of Leader Oscillator
...
def Get_Dist_Vel(previous_distance, elapsed_time, control_input):
    ...
    x_states = np.array([distance, control_input]) # np.array([distance, velocity])
...
def Agent_j_Data_Callback(xbee_message):
    ...
    # Assign Values
    x_states_j = np.array([data[0], data[1]])
    phi_jk = np.array([data[2], data[3], data[4]])
    phi_jkplus1 = np.array([data[2], data[3], data[4]])
    theta_j = np.array([data[5], data[6], data[7]])
    reward_j = data[8]
    done_j = data[9]
...
def Send_Agent_i_State(x_i, phi, theta, reward, done, local_device, remote_device):
    if done == True:
        done = 1
    else:
        done = 0
    data = np.array([x_i[0],x_i[1],phi[0],phi[1],phi[2],theta[0],theta[1],theta[2],reward,done])
...
def action(theta_k, x_i, x_j, x_leader, Nu):
    exponent = -(np.power(np.linalg.norm(x_i[0] - x_j[0]), 2) + np.power(np.linalg.norm(x_i[0] - x_leader[0]), 2))/(np.power(Nu[0],2))
    num = theta_k[1] * np.exp(exponent)
    den = 2 * theta_k[2]
    summ = (x_j[0] - x_i[0]) + (x_leader[0] - x_i[0])
...
def Phi(x_i, x_j, x_leader, Nu, u_i):
    # Phi 1
    num_1 = -(np.power(np.linalg.norm(x_i[0] - x_j[0]), 2) + np.power(np.linalg.norm(x_i[0] - x_leader[0]), 2))
    den_1 = np.power(Nu[0],2)
    summ_1 = np.power(np.linalg.norm(x_i[0] - x_j[0]), 2) + np.power(np.linalg.norm(x_i[0] - x_leader[0]), 2)
    phi_1 = np.exp(num_1/den_1) * summ_1
    
    # Phi 2
    num_2 = -(np.power(np.linalg.norm(x_i[0] - x_j[0]), 2) + np.power(np.linalg.norm(x_i[0] - x_leader[0]), 2))
    den_2 = np.power(Nu[1],2)
    summ_2 = ((x_i[0] - x_j[0]) * u_i) + ((x_i[0] - x_leader[0]) * u_i)
...
### XBEE SETUP
local_device = XBeeDevice("/dev/ttyUSB0", 9600) # Orange XBee
local_device.open()
...
# Get Initial States
x_i.append(Get_Dist_Vel(0, 0, 0))
Send_Agent_i_State(x_i[k], np.array([0,0,0]), theta[k], 0, False, local_device, remote_device)
...
print("Connection Established ... Program Starting")
...
while not done:
    ...
    # Get Phi
    phi.append(Phi(x_i[k], x_j[k], x_leader[k], Nu, u[k])) # phi(k)
    ...
    x_i.append(Get_Dist_Vel(x_i[k][0], elapsed_time, u[k])) # x_i(k+1)
    
    # Update Leader States
    if np.remainder(k,5) == 0: # Wait until data received
        Leader_Control = -Leader_Control
    u_leader.append(Leader_Control) # u_leader(k) # 0.02 0.1
    x_leader.append(Apply_Leader_Agent_Action(x_leader[k], u_leader[k], 1)) # x_leader(k+1)
    start_time = time.time()

    # Send Updated State
    Send_Agent_i_State(x_i[k + 1], phi[k], theta[k], 0, done, local_device, remote_device)
    
    # Get Info from other Agent
    while data_received_flag is False: # Wait until data received
        wait_time = time.time()
    
    phi_j.append(phi_jk) # phi_j[k]
    x_j.append(x_states_j) # x_j(k+1)
    data_received_flag = False
    
    # Get Reward
    reward.append(Get_Reward(x_i[k], x_j[k], x_leader[k], u[k], Gamma, Lambda)) # r(k)
    ...
    Send_Agent_i_State(x_i[k + 1], phi_predict[k], theta[k], reward[k], done, local_device, remote_device)
    ...
    theta.append(Theta_Update(theta[k], theta_j, LEARNING_RATE, reward[k], reward_j, phi_predict[k], phi[k], phi_jkplus1, phi_j[k])) # theta(k+1)
